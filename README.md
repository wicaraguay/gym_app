...
La app Expo se puede arrancar desde la raiz del repo con:

```bash
npx expo start
```

Expo usa la entrada de `mobile/index.ts` y la configuracion compartida de la
raiz. Si prefieres trabajar directo dentro de `mobile/`, tambien sigue
funcionando `cd mobile && npx expo start`.
