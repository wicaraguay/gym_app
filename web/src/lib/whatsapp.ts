...
// wa.me solo PRE-LLENA el mensaje (no lo envia solo): el dueno lo revisa y toca
// enviar. Los textos estan centralizados aca para ajustarlos en un solo lugar.

export type ReminderKind =
  | 'saldo' // tiene saldo pendiente -> recordatorio de cobro
  | 'vencida' // membresia vencida -> invitacion a renovar
  | 'por_vencer' // vence pronto (<= 7 dias) -> aviso de vencimiento
  | 'al_dia' // vigente y al dia -> mensaje motivacional
  | 'sin_membresia'; // no tiene membresia -> invitacion a arrancar

export interface ReminderInput {
  firstName: string;
  businessName?: string;
  saldoTotal: number;
  activeEndDate?: string | null; // vencimiento de la membresia vigente (ISO) o null
  lastExpiredEndDate?: string | null; // vencimiento de la ultima vencida (ISO) o null
...
// Decide el estado MAS relevante (prioridad: cobro > vencida > por vencer > al dia).
...
// Etiqueta corta para mostrar en la UI que tipo de aviso se va a enviar.
...
export function buildReminderMessage(i: ReminderInput, kind: ReminderKind): string {
  ...
  switch (kind) {
    case 'saldo': {
      // Si ademas tiene vencimiento a la vista, lo sumamos como contexto.
